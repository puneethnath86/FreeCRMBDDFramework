...
//		Initializing credentials variable that is of type DataTable
		...
		for(Map<String, String> loginDataMap : credentials.asMaps(String.class, String.class)) {
			driver.findElement(By.xpath("//input[@placeholder='E-mail address']")).sendKeys(loginDataMap.get("username"));
			driver.findElement(By.xpath("//input[@placeholder='Password']")).sendKeys(loginDataMap.get("password"));
		}
	...
	public void user_will_enter_deal_details(DataTable dealsData){
		...
		for(Map<String, String> dealsDataValues : dealsData.asMaps(String.class, String.class)) {
//			Getting data from Test data table from 0th row and 0 to 3rd element of the data table from features file
			driver.findElement(dealNameLoc).sendKeys(dealsDataValues.get("dealtitle"));
			driver.findElement(amountLoc).sendKeys(dealsDataValues.get("amount"));
			driver.findElement(probablityLoc).sendKeys(dealsDataValues.get("probablity"));
			driver.findElement(commissionLoc).sendKeys(dealsDataValues.get("commission"));
